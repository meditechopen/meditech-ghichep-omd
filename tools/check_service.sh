...
#STATUS_MSG=$(eval "$SERVICETOOL" | sed -n '1p;3p' 2>&1)
STATUS_MSG1=$(eval "$SERVICETOOL" | sed -n '1p' |  awk '{print $2}' 2>&1)
STATUS_MSG2=$(eval "$SERVICETOOL" | sed -n '3p' |  awk '{print $2}' 2>&1)
STATUS_MSG="${STATUS_MSG1} is ${STATUS_MSG2}"
