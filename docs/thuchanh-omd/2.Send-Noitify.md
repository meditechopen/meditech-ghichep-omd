...
Trong ví dụ này, tôi sẽ thêm **Rule** với **Description** là `Send mail for services when status change` (Điền tùy ý) và **User** nhận bản tin cảnh báo là `User1` vì ở phần trên tôi đã hướng dẫn gửi cho [nhóm](#group), cho service `SSH` (Để áp dụng rule cho tất cả các service, vui lòng không tick vào ô.) cho tất cả các host (Không tích vào ô `Match only following hosts`) 
...
Tối thiểu, chúng ta phải tạo 2 nhóm để có thể kiểm tra tính năng này. Vui lòng tham khảo phần tạo [Group](#21) và thêm [User vào Group](#22) để tạo mới một **group** mới là `group2` và **user** có tên là `User Support 2`.
